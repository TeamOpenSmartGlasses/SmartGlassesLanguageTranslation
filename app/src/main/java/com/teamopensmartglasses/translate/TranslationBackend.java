...
public class TranslationBackend {
  ...
  @Subscribe
  public void onRequestTranslateMessagEvent(RequestTranslateMessageEvent event){
    // Also update srcTextView with targetText
    Log.d("TAG", "MESSAGE FIRED");
    sourceText.setValue(event.message);
  }
        ...
        .continueWithTask(
            new Continuation<Void, Task<String>>() {
              ...
              public Task<String> then(@NonNull Task<Void> task) {
                if (task.isSuccessful()) {
                  return translators.get(options).translate(text).addOnCompleteListener(processTranslation);
                          /*
                          .addOnCompleteListener(
                          new OnCompleteListener<String>() {
                            @Override
